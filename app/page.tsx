  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 p-4 pb-24 transition-colors duration-300 ease-in-out select-none">
      <header className="mb-6 mt-2 flex justify-between items-start">
        ...
        {mounted && (
            <button onClick={() => { setTheme(theme === 'dark' ? 'light' : 'dark'); toast.success(`Mode ${theme === 'dark' ? 'Terang' : 'Gelap'} diaktifkan!`, { icon: theme === 'dark' ? '☀️' : '🌙' }) }} className="p-3 bg-white dark:bg-slate-800 rounded-full shadow-sm border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 hover:scale-105 active:scale-95 transition-all">
      ...
      <Link href="/report" className="block mb-3">
        <div className="bg-gradient-to-br from-pop-green to-emerald-600 p-5 rounded-2xl text-white shadow-lg shadow-green-200 dark:shadow-none cursor-pointer hover:scale-[1.02] active:scale-[0.98] transition-transform">
          <div className="flex items-center gap-2 mb-2 opacity-90"><DollarSign size={20} /><span className="text-sm font-medium">Omset Hari Ini</span></div>
          <div className="text-3xl font-bold">{loading ? '...' : `Rp ${stats.dailyIncome.toLocaleString()}`}</div>
          <div className="text-[10px] mt-2 bg-white/20 inline-block px-2 py-0.5 rounded-full backdrop-blur-sm">Klik untuk lihat laporan lengkap</div>
        </div>
      </Link>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <Link href="/transactions">
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 h-full flex flex-col justify-between hover:bg-blue-50 dark:hover:bg-slate-700/50 active:scale-[0.98] transition-all group">
            <div className="flex items-center gap-2 text-blue-500 mb-2"><TrendingUp size={18} className="group-hover:scale-110 transition-transform"/><span className="text-[11px] font-bold uppercase tracking-wider">Transaksi</span></div>
            <div><div className="text-xl font-bold text-gray-800 dark:text-white transition-colors">{loading ? '...' : stats.totalTransactions}</div><p className="text-[10px] text-gray-400">Order masuk hari ini</p></div>
          </div>
        </Link>
        <Link href="/inventory">
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 h-full flex flex-col justify-between hover:bg-orange-50 dark:hover:bg-slate-700/50 active:scale-[0.98] transition-all group">
            <div className="flex items-center gap-2 text-orange-500 mb-2"><Package size={18} className="group-hover:scale-110 transition-transform"/><span className="text-[11px] font-bold uppercase tracking-wider">Inventory</span></div>
            <div><div className="text-xl font-bold text-gray-800 dark:text-white transition-colors">{loading ? '...' : stats.totalProducts}</div><p className="text-[10px] text-gray-400">Total item aktif</p></div>
          </div>
        </Link>
        <Link href="/inventory/audit" className="col-span-2">
          <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex items-center justify-between hover:bg-purple-50 dark:hover:bg-slate-700/50 active:scale-[0.98] transition-all">
            <div className="flex items-center gap-3"><div className="p-2 bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-lg"><ClipboardList size={20} /></div><div><div className="font-bold text-sm text-gray-800 dark:text-white transition-colors">Riwayat Opname</div><div className="text-[10px] text-gray-500">Log perubahan & selisih stok</div></div></div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden transition-colors duration-300">
        <div className="p-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center bg-gray-50/50 dark:bg-slate-800/50">
            <div className="flex items-center gap-2 text-gray-800 dark:text-white font-bold text-sm"><Award size={18} className="text-yellow-500" /> Produk Terlaris</div>
            <span className="text-[10px] text-gray-500 bg-gray-200 dark:bg-slate-700 px-2 py-1 rounded transition-colors">All Time</span>
        </div>
        <div className="p-2">
            {loading ? <div className="text-center py-6 text-xs text-gray-400">Menganalisa data...</div> : topProducts.length === 0 ? <div className="text-center py-6 text-xs text-gray-400">Belum ada data penjualan.</div> : (
                topProducts.map((product, index) => (
                    <div key={index} className="flex items-center justify-between p-3 border-b last:border-0 border-gray-50 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30 rounded-lg transition-colors">
                        <div className="flex items-center gap-3">
                            <span className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${index === 0 ? 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400' : index === 1 ? 'bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-300' : index === 2 ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400' : 'bg-gray-100 text-gray-400 dark:bg-slate-800 dark:text-gray-500'}`}>{index + 1}</span>
                            <span className="font-medium text-sm text-gray-700 dark:text-gray-200 line-clamp-1 transition-colors">{product.name}</span>
                        </div>
                        <div className="font-bold text-sm text-pop-green whitespace-nowrap bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-lg">{product.qty} <span className="text-[10px] text-green-600/70 dark:text-green-400/70 font-normal ml-0.5">Terjual</span></div>
                    </div>
                ))
            )}
        </div>
