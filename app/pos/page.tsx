...
export default function POS() {
  ...
  // --- HELPER: HITUNG TOTAL STOK TERPAKAI DI KERANJANG (Global per Produk) ---
  const getTotalQtyInCart = (productId: number) => {
    return cart.filter(item => item.id === productId).reduce((sum, item) => sum + item.quantity, 0)
  }

  // --- 1. ADD TO CART (Smart Merge) ---
  const addToCart = (product: any) => {
    // Cek total stok terpakai di seluruh keranjang
    ...
    setCart(prev => {
      // Cari item yang SAMA persis (ID sama DAN Harga sama/tidak gratis)
      // Kita hanya merge ke item yang bayar (price > 0). Item gratis biarkan terpisah.
      const existingPaidItemIndex = prev.findIndex(p => p.id === product.id && p.price !== 0)

      if (existingPaidItemIndex >= 0) {
        // Kalau sudah ada baris item bayar, tambahkan qty di situ
        const newCart = [...prev]
        newCart[existingPaidItemIndex].quantity += 1
        return newCart
      }

      // Kalau belum ada, buat baris baru dengan unique ID (cartId)
      return [...prev, { 
        ...product, 
        quantity: 1, 
        original_price: product.price,
        cartId: Date.now() + Math.random() // Unique ID untuk setiap baris
      }]
    })
  }

  // --- 2. UPDATE QTY (Dengan Cek Stok Global) ---
  const updateQuantity = (cartId: string, delta: number) => {
    setCart(prev => {
      const item = prev.find(p => p.cartId === cartId)
      if (!item) return prev

      // Jika menambah, cek stok global dulu
      if (delta > 0) {
      ...
      return prev.map(p => {
        if (p.cartId === cartId) {
          ...
          if (newQty < 1) return p // Minimal 1, pakai tombol hapus kalau mau buang
  ...
  // --- 3. TOGGLE FREE ITEM (Fitur Split Otomatis) ---
  const toggleFreeItem = (cartId: string) => {
    setCart(prev => {
      ...
      // KASUS A: Mau meng-GRATIS-kan (Bayar -> Gratis)
      if (!isCurrentlyFree) {
        // Jika Qty > 1 (Misal ada 2 Kopi), kita PECAH.
        // 1 Jadi gratis (baris baru), sisanya tetap bayar.
        if (item.quantity > 1) {
          ...
          const newItemFree = { 
            ...
            quantity: 1, 
            price: 0, 
            cartId: Date.now() + Math.random() // ID Baru
          }
          
          // Ganti array: Hapus item lama, masukkan 2 item baru (Split)
          const newCart = [...prev]
          newCart.splice(targetIndex, 1, newItemPaid, newItemFree)
          return newCart
        } 
        
        // Jika Qty cuma 1, langsung ubah jadi Rp 0
        const newCart = [...prev]
        newCart[targetIndex] = { ...item, price: 0 }
        return newCart
      } 
      
      // KASUS B: Mau mem-BAYAR-kan kembali (Gratis -> Bayar)
      else {
        // Balikin harga ke original
        const newCart = [...prev]
  ...
  // ... Logic Kamera & Checkout (Tidak Berubah) ...
  const handleCameraCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    const fileList = e.target.files
    
    // Pastikan fileList ada dan isinya tidak kosong
    if (fileList && fileList[0]) {
      const file = fileList[0] // <--- AMBIL FILE DULU KE VARIABEL
      ...
      // Masukkan variabel 'file' ke state (jangan pakai e.target di sini)
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Update Stock (Looping aman karena ID produk tetap sama walau dipecah)
    ...
    <div className="pb-52 bg-gray-50 dark:bg-slate-900 min-h-screen transition-colors">
      ...
      <div className="p-4 space-y-3">
        ...
        {cart.map((item) => {
           ...
           // Gunakan cartId sebagai KEY React agar tidak error saat di-split
           return (
      ...
      {/* Footer Area (Sama seperti sebelumnya) */}
      <div className="fixed bottom-16 w-full bg-white dark:bg-slate-800 border-t dark:border-slate-700 p-4 shadow-[0_-5px_15px_rgba(0,0,0,0.1)] rounded-t-2xl z-30 transition-colors">
        <div className="grid grid-cols-2 gap-3 mb-3">
           <input placeholder="Lokasi Event" className="border p-2 rounded text-sm bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white" value={location} onChange={e => setLocation(e.target.value)} />
        ...
        <div className="flex gap-2 mb-3">
            <select className="border p-2 rounded text-sm bg-gray-50 dark:bg-slate-700 dark:border-slate-600 dark:text-white w-20" value={discountType} onChange={e => setDiscountType(e.target.value)}>
        ...
        <div className="mb-3">
            <div className="flex gap-2 overflow-x-auto pb-2 items-center no-scrollbar">
              ...
              {proofFiles.map((file, idx) => {
                // PENGECEKAN KEAMANAN: Jika file invalid/bukan object, jangan render
                if (!file || typeof file !== 'object') return null

                // Buat URL dengan aman
                let imgUrl = ''
                try {
                ...
                return (
                  <div key={idx} className="relative w-16 h-16 flex-shrink-0 bg-gray-200 rounded-lg overflow-hidden border dark:border-slate-600">
                    <img 
                      ...
                      alt="preview" 
                      // Cleanup URL saat gambar selesai load untuk mencegah memory leak (opsional tapi bagus)
        ...
        <div className="flex flex-col gap-1 mb-3 pt-2 border-t dark:border-slate-700">
           {discountValue > 0 && <div className="flex justify-between text-sm text-red-500"><span>Diskon Tambahan</span><span>- Rp {discountAmount.toLocaleString()}</span></div>}
           <div className="flex justify-between items-center font-bold text-xl dark:text-white"><span>Total</span><span>Rp {total.toLocaleString()}</span></div>
        </div>
        <button onClick={handleCheckout} disabled={loading || cart.length === 0} className="w-full bg-pop-green hover:bg-pop-green-dark text-white py-3 rounded-xl font-bold text-lg disabled:bg-gray-300 dark:disabled:bg-slate-600 transition-colors">{loading ? 'Memproses...' : 'Bayar Sekarang'}</button>
      </div>
    </div>
  )
}