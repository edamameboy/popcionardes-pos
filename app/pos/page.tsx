...
export default function POS() {
  ...
  // STATE PENTING: Untuk mendeteksi apakah kamera scanner sedang nyala?
    ...
    setCart(prev => {
      ...
      if (!isCurrentlyFree) {
        if (item.quantity > 1) {
          ...
          const newItemFree = { ...item, quantity: 1, price: 0, cartId: Date.now() + Math.random() }
          const newCart = [...prev]
  ...
  // --- CHECKOUT (Hybrid Online/Offline) ---
  const handleCheckout = async () => {
    ...
    try {
      ...
      const isNetworkError = error.message === 'OFFLINE_MODE' || error.message.includes('fetch') || error.message.includes('network') || error.message.includes('connection');
  ...
  return (
    <div className="pb-52 bg-gray-50 dark:bg-slate-900 min-h-screen transition-colors">
      ...
      {/* COMPONENT INPUT PRODUK & SCANNER */}
      {/* Kita oper 'setIsScanning' ke sini agar ProductInput bisa memberi kabar kalau kamera lagi nyala */}
      <ProductInput 
        onAddProduct={addToCart} 
        onScanStateChange={(isActive) => setIsScanning(isActive)} 
      />
      ...
      {/* --- PANEL PEMBAYARAN (FOOTER) --- */}
      {/* LOGIKA UTAMA ADA DI SINI:
          Jika isScanning = true (lagi scan), maka class 'translate-y-full' aktif (panel turun ke bawah layar).
          Jika isScanning = false, panel muncul lagi.
      */}
      <div className={`fixed bottom-16 w-full bg-white dark:bg-slate-800 border-t dark:border-slate-700 p-4 shadow-[0_-5px_15px_rgba(0,0,0,0.1)] rounded-t-2xl z-30 transition-all duration-300 ${isScanning ? 'translate-y-full opacity-0 pointer-events-none' : 'translate-y-0 opacity-100'}`}>
